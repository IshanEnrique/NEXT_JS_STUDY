...
const Tshirts = () => {
  const [tshirts, setTshirts] = useState([]);
    ...
    const getTShirts = async () => {
        let tShirtsRes = await fetch("/api/getAllTShirts")
        if (tShirtsRes.status == 200) {
            let tshirtsJson = await tShirtsRes.json();
            setTshirts(tshirtsJson);

        } else {
            console.info(">>>>>>>>Get TShirts API Error response  : " + tShirtsRes);
          ...
          <div className="flex flex-wrap -m-4">
            {
              tshirts.map((tshirt)=>{
                ...
                return (
                <Link key={tshirt.productCode} href={tshirt.href} legacyBehavior><a><div className="lg:w-full md:w-full p-4 w-full shadow-lg m-5">
                ...
                <span key={element} className="mt-1 mx-2" >
                  {element}
                  <input className="mt-2 pt-1 mx-1"   type="checkbox" name="size" id="size" checked={false} value={element} />
                </span> 
                    )
